...
@kappiebasicRegex = ""
@ccp_apiserverRegex = ""
@ccp_kubecontrollermanagerRegex = ""
@ccp_kubeschedulerRegex = ""
@ccp_clusterautoscalerRegex = ""
@ccp_etcdRegex = ""
...
@ccp_kubecontrollermanagerRegex_minimal = "rest_client_requests_duration_seconds_bucket|rest_client_requests_total|workqueue_depth|node_collector_evictions_total"
@ccp_kubeschedulerRegex_minimal = "scheduler_pending_pods|scheduler_unschedulable_pods|scheduler_pod_scheduling_attempts|scheduler_queue_incoming_pods_total|scheduler_preemption_attempts_total|scheduler_preemption_victims|scheduler_scheduling_attempt_duration_seconds|Scheduler_schedule_attempts_total|scheduler_pod_scheduling_duration_seconds"
@ccp_apiserverRegex_minimal = "apiserver_request_total|apiserver_cache_list_fetched_objects_total|apiserver_cache_list_returned_objects_total|apiserver_flowcontrol_demand_seats_average|apiserver_flowcontrol_current_limit_seats|apiserver_flowcontrol_rejected_requests_total|apiserver_request_sli_duration_seconds|process_start_time_seconds|apiserver_request_duration_seconds|apiserver_storage_fetched_objects_total|apiserver_storage_list_returned_objects_total|apiserver_current_inflight_requests"
@ccp_clusterautoscalerRegex_minimal = "rest_client_requests_total|cluster_autoscaler_((last_activity|cluster_safe_to_autoscale|failed_scale_ups_total|scale_down_in_cooldown|scaled_up_nodes_total|unneeded_nodes_count|unschedulable_pods_count|nodes_count))|cloudprovider_azure_api_request_(errors|duration_seconds_(bucket|count))"
@ccp_etcdRegex_minimal = "etcd_memory_in_bytes|etcd_cpu_in_cores|etcd_db_limit_in_bytes|etcd_db_max_size_in_bytes|etcd_db_fragmentation_rate|etcd_db_total_object_count|etcd_db_top_N_object_counts_by_type|etcd_db_top_N_object_size_by_type|etcd2_enabled"
...
@ccp_kubecontrollermanagerRegex_minimal_mac = "rest_client_requests_duration_seconds_bucket|rest_client_requests_total|workqueue_depth|node_collector_evictions_total"
@ccp_kubeschedulerRegex_minimal_mac = "scheduler_pending_pods|scheduler_unschedulable_pods|scheduler_pod_scheduling_attempts|scheduler_queue_incoming_pods_total|scheduler_preemption_attempts_total|scheduler_preemption_victims|scheduler_scheduling_attempt_duration_seconds|Scheduler_schedule_attempts_total|scheduler_pod_scheduling_duration_seconds"
@ccp_apiserverRegex_minimal_mac = "apiserver_request_total|apiserver_cache_list_fetched_objects_total|apiserver_cache_list_returned_objects_total|apiserver_flowcontrol_demand_seats_average|apiserver_flowcontrol_current_limit_seats|apiserver_flowcontrol_rejected_requests_total|apiserver_request_sli_duration_seconds|process_start_time_seconds|apiserver_request_duration_seconds|apiserver_storage_fetched_objects_total|apiserver_storage_list_returned_objects_total|apiserver_current_inflight_requests"
...
def populateSettingValuesFromConfigMap(parsedConfig)
  ...
  ccp_kubecontrollermanagerRegex = parsedConfig[:"controlplane-kube-controller-manager"]
  if !ccp_kubecontrollermanagerRegex.nil? && ccp_kubecontrollermanagerRegex.kind_of?(String)
    if !ccp_kubecontrollermanagerRegex.empty?
      if isValidRegex(ccp_kubecontrollermanagerRegex) == true
        @ccp_kubecontrollermanagerRegex = ccp_kubecontrollermanagerRegex
        ConfigParseErrorLogger.log(LOGGING_PREFIX, "Using configmap metrics keep list regex for kube-controller-manager")
      else
        ConfigParseErrorLogger.logError(LOGGING_PREFIX, "Invalid keep list regex for kube-controller-manager")
      end
    end
  else
    ConfigParseErrorLogger.logError(LOGGING_PREFIX, "ccp_kubecontrollermanagerRegex either not specified or not of type string")
  end

  ccp_kubeschedulerRegex = parsedConfig[:"controlplane-kube-scheduler"]
  if !ccp_kubeschedulerRegex.nil? && ccp_kubeschedulerRegex.kind_of?(String)
    if !ccp_kubeschedulerRegex.empty?
      if isValidRegex(ccp_kubeschedulerRegex) == true
        @ccp_kubeschedulerRegex = ccp_kubeschedulerRegex
        ConfigParseErrorLogger.log(LOGGING_PREFIX, "Using configmap metrics keep list regex for kube-scheduler")
      else
        ConfigParseErrorLogger.logError(LOGGING_PREFIX, "Invalid keep list regex for kube-scheduler")
      end
    end
  else
    ConfigParseErrorLogger.logError(LOGGING_PREFIX, "ccp_kubeschedulerRegex either not specified or not of type string")
  end

  ccp_apiserverRegex = parsedConfig[:"controlplane-apiserver"]
  if !ccp_apiserverRegex.nil? && ccp_apiserverRegex.kind_of?(String)
    ...
    ConfigParseErrorLogger.logError(LOGGING_PREFIX, "ccp_apiserverRegex either not specified or not of type string")
  end

  ccp_clusterautoscalerRegex = parsedConfig[:"controlplane-cluster-autoscaler"]
  if !ccp_clusterautoscalerRegex.nil? && ccp_clusterautoscalerRegex.kind_of?(String)
    if !ccp_clusterautoscalerRegex.empty?
      if isValidRegex(ccp_clusterautoscalerRegex) == true
        @ccp_clusterautoscalerRegex = ccp_clusterautoscalerRegex
    ...
    ConfigParseErrorLogger.logError(LOGGING_PREFIX, "ccp_clusterautoscalerRegex either not specified or not of type string")
  end

  ccp_etcdRegex = parsedConfig[:"controlplane-etcd"]
  if !ccp_etcdRegex.nil? && ccp_etcdRegex.kind_of?(String)
    if !ccp_etcdRegex.empty?
      if isValidRegex(ccp_etcdRegex) == true
        @ccp_etcdRegex = ccp_etcdRegex
    ...
    ConfigParseErrorLogger.logError(LOGGING_PREFIX, "ccp_etcdRegex either not specified or not of type string")
  end
  ...
  begin
    if @minimalIngestionProfile == "true"
      if @isMacMode == true
        ...
        @ccp_kubecontrollermanagerRegex = @ccp_kubecontrollermanagerRegex + "|" + @ccp_kubecontrollermanagerRegex_minimal_mac
        @ccp_kubeschedulerRegex = @ccp_kubeschedulerRegex + "|" + @ccp_kubeschedulerRegex_minimal_mac
        @ccp_apiserverRegex = @ccp_apiserverRegex + "|" + @ccp_apiserverRegex_minimal_mac
        @ccp_clusterautoscalerRegex = @ccp_clusterautoscalerRegex + "|" + @ccp_clusterautoscalerRegex_minimal_mac
        @ccp_etcdRegex = @ccp_etcdRegex + "|" + @ccp_etcdRegex_minimal_mac
      else
        ...
        @ccp_kubecontrollermanagerRegex = @ccp_kubecontrollermanagerRegex + "|" + @ccp_kubecontrollermanagerRegex_minimal
        @ccp_kubeschedulerRegex = @ccp_kubeschedulerRegex + "|" + @ccp_kubeschedulerRegex_minimal
        @ccp_apiserverRegex = @ccp_apiserverRegex + "|" + @ccp_apiserverRegex_minimal
        @ccp_clusterautoscalerRegex = @ccp_clusterautoscalerRegex + "|" + @ccp_clusterautoscalerRegex_minimal
        @ccp_etcdRegex = @ccp_etcdRegex + "|" + @ccp_etcdRegex_minimal
      end
    end
  rescue => errorStr
